...
There is, however, one case where we still may transform the LHS of a rule:
postInlineUnconditionally. For instance, in the case of
...
that a rule template contained ticks in its arguments.
See Note [Tick annotations in RULE matching] in GHC.Core.Rules for details.
